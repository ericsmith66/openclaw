# PRD — Agent Task Log Requirements

## Purpose
Create a consistent, reviewable, and commit-friendly way for AI coding agents to record **what was done for a specific task** into the repository so the team can track decisions, changes, commands, and outcomes over time.
...
### Directory
All agent task logs MUST be stored in:
- `knowledge_base/task-logs/`
...
- It exists in `knowledge_base/task-logs/` with correct naming
...
- Add a "Screenshots" section for UI tasks (store images under `knowledge_base/task-logs/assets/` if needed).

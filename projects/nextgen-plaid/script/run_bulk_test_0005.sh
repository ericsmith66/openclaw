...
RAG_PATH="knowledge_base/static_docs/eric_grok_static_rag.md"

EPIC_CONTENT="$(ruby -e 'print File.read(ARGV[0])' "$EPIC_PATH")"
RAG_CONTENT="$(ruby -e 'print File.read(ARGV[0])' "$RAG_PATH")"
...
KB-only.
Include a section titled "Context Used" with citations to ${EPIC_PATH} and ${RAG_PATH} sections.
...
EOF
  ...
  jq -n \
    ...
    '{
      model: $model,
      messages: [
        {role:"system", content:("Context (AGENT-05 Epic):\n" + $epic + "\n\nContext (Project RAG):\n" + $rag)},

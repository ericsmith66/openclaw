...
RSpec.describe GrokClient do
  ...
  let(:api_key) { 'test-grok-key' }
  let(:client)  { described_class.new(api_key: api_key) }

  # Effective URL: BASE_URL is 'https://api.x.ai/v1', GET path is '/v1/models'
  # Faraday merges base + path → 'https://api.x.ai/v1/models'
  let(:models_url) { 'https://api.x.ai/v1/models' }

  # Minimal but realistic xAI /v1/models response body
  let(:api_model_payload) do
    {
      'object' => 'list',
      'data'   => [
        { 'id' => 'grok-3',      'object' => 'model', 'owned_by' => 'xai', 'created' => 1_700_000_000 },
        { 'id' => 'grok-3-mini', 'object' => 'model', 'owned_by' => 'xai', 'created' => 1_700_000_001 }
      ]
    }
  end

  # Keys every normalised model hash must carry (T5 contract)
  REQUIRED_MODEL_KEYS = %i[id object owned_by created smart_proxy].freeze
  ...
  describe '#list_models (T1: 200 success)' do
    ...
    it 'maps id from the upstream payload' do
      ...
      expect(ids).to contain_exactly('grok-3', 'grok-3-mini')
    end
    ...
    it 'preserves owned_by from upstream when present' do
      expect(client.list_models.map { |m| m[:owned_by] }).to all(eq('xai'))
    end

    it 'defaults owned_by to "xai" when upstream omits the field' do
      payload_no_owner = {
        'object' => 'list',
        'data'   => [{ 'id' => 'grok-unknown', 'object' => 'model', 'created' => 1_700_000_002 }]
      }
      stub_request(:get, models_url)
        .to_return(status: 200, body: payload_no_owner.to_json,
                   headers: { 'Content-Type' => 'application/json' })

      expect(described_class.new(api_key: api_key).list_models.first[:owned_by]).to eq('xai')
    end
    ...
    it 'sets smart_proxy provider to "xai"' do
      expect(client.list_models.map { |m| m.dig(:smart_proxy, :provider) }).to all(eq('xai'))
    end
  ...
  describe '#list_models (T2: Faraday::Error)' do
    context 'when Faraday::ConnectionFailed is raised' do
      ...
      it 'returns an Array' do
        with_env('GROK_MODELS', nil) { expect(client.list_models).to be_an(Array) }
      end

      it 'returns [] when GROK_MODELS is not set' do
        with_env('GROK_MODELS', nil) { expect(client.list_models).to eq([]) }
      end

      it 'does not propagate the exception' do
        with_env('GROK_MODELS', nil) { expect { client.list_models }.not_to raise_error }
      end
    end
  ...
  describe '#list_models (T3: non-200 response)' do
    [401, 403, 429, 500, 503].each do |status_code|
      context "when the server returns HTTP #{status_code}" do
        ...
        it "returns [] for #{status_code} when GROK_MODELS is not set" do
          with_env('GROK_MODELS', nil) { expect(client.list_models).to eq([]) }
        end
  ...
  # -----------------------------------------------------------------------
  # T4 – falls back to GROK_MODELS CSV env var when live returns empty
  # -----------------------------------------------------------------------

  describe '#list_models (T4: GROK_MODELS env var fallback)' do
    let(:env_models_csv) { 'grok-3,grok-3-mini, grok-4 ' }

    shared_examples 'returns env fallback models' do
      it 'returns an Array' do
        with_env('GROK_MODELS', env_models_csv) { expect(client.list_models).to be_an(Array) }
      end

      it 'returns one entry per non-empty CSV token' do
        with_env('GROK_MODELS', env_models_csv) { expect(client.list_models.size).to eq(3) }
      end

      it 'uses the CSV values as model ids' do
        with_env('GROK_MODELS', env_models_csv) do
          expect(client.list_models.map { |m| m[:id] })
            .to contain_exactly('grok-3', 'grok-3-mini', 'grok-4')
        end
      end

      it 'strips surrounding whitespace from each CSV token' do
        with_env('GROK_MODELS', ' grok-3 , grok-4 ') do
          expect(client.list_models.map { |m| m[:id] })
            .to contain_exactly('grok-3', 'grok-4')
        end
      end

      it 'sets owned_by to "xai" for every fallback entry' do
        with_env('GROK_MODELS', env_models_csv) do
          expect(client.list_models.map { |m| m[:owned_by] }).to all(eq('xai'))
        end
      end

      it 'sets smart_proxy provider to "xai" for every fallback entry' do
        with_env('GROK_MODELS', env_models_csv) do
          expect(client.list_models.map { |m| m.dig(:smart_proxy, :provider) }).to all(eq('xai'))
        end
      end
    ...
    context 'when GROK_MODELS is an empty string' do
      before do
        stub_request(:get, models_url)
          .to_raise(Faraday::ConnectionFailed.new('connection refused'))
      end
      ...
      it 'returns [] (empty CSV treated as no fallback)' do
        with_env('GROK_MODELS', '') { expect(client.list_models).to eq([]) }
      end
    end
  end
  ...
  describe '#list_models (T5: normalised key contract)' do
    context 'when the API returns live models' do
      ...
      it 'every hash contains all five required keys' do
        client.list_models.each do |model|
          expect(model.keys).to include(*REQUIRED_MODEL_KEYS),
            "expected model #{model.inspect} to include #{REQUIRED_MODEL_KEYS}"
        end
      end
    ...
    context 'when models come from the GROK_MODELS env var fallback' do
      before do
        stub_request(:get, models_url)
          .to_raise(Faraday::ConnectionFailed.new('connection refused'))
      end
      ...
      it 'every fallback hash contains all five required keys' do
        with_env('GROK_MODELS', 'grok-3,grok-3-mini') do
          client.list_models.each do |model|
            expect(model.keys).to include(*REQUIRED_MODEL_KEYS),
              "expected fallback model #{model.inspect} to include #{REQUIRED_MODEL_KEYS}"
          end
        end
      end

      it 'fallback created is an Integer' do
        with_env('GROK_MODELS', 'grok-3') do
          client.list_models.each { |m| expect(m[:created]).to be_an(Integer) }
        end
      end

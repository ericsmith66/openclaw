...
class SmartProxyApp < Sinatra::Base
  ...
  post '/v1/chat/completions' do
    ...
    # Claude streaming is handled internally by SmartProxy (ClaudeClient does not support
    # raw SSE passthrough — it parses the response body as JSON). Force non-streaming
    # upstream for all Claude requests; SmartProxy converts the response to SSE if needed.
    if request_payload['stream'] == true && routing[:provider] == :claude
      upstream_payload['stream'] = false
    end

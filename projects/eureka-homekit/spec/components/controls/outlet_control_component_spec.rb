...
RSpec.describe Controls::OutletControlComponent, type: :component do
  let(:home) { Home.create!(name: "Test Home", uuid: "home-123") }
  let(:room) { Room.create!(name: "Test Room", uuid: "room-123", home: home) }
  let(:accessory) { Accessory.create!(name: "Test Outlet", uuid: "acc-456", room: room, last_seen_at: 5.minutes.ago) }
  ...
  let(:outlet_in_use_sensor) do
    Sensor.create!(
      accessory: accessory,
      characteristic_type: "Outlet In Use",
      current_value: "0",
      service_uuid: "svc-456",
  ...
  describe "rendering" do
    it "renders the accessory name" do
      ...
      expect(rendered_content).to include("Test Outlet")
    end
